...
angular.module('app', ['ngRoute', 'angularMoment', 'epCore']).run(function (amMoment) {
...
.controller('MainController', function ($scope, $rootScope, $location, epService) {
  ...
  $scope.delayOptions = [10, 20, 30, 60, 120];

  $scope.onChange = function (t) {
    if (t === 'auto') {
      chrome.storage.sync.set({
        'auto': $scope.conf.auto,
        'delay': $scope.conf.delay
      }, function () {
        chrome.runtime.sendMessage({
          type: 'auto'
        });
      ...
      chrome.storage.sync.set({
        'check': $scope.conf.check
      });
    } else if (t === 'sync') {
      chrome.storage.sync.set({
        'sync': $scope.conf.sync
      }, function (conf) {
        chrome.runtime.sendMessage({
          type: 'sync'
        });
